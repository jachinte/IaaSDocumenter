...
public class InfrastructureClient {
    ...
    private Exception awsException;
    ...
    public List<InternetGateway> getInternetGateways(Vpc vpc) {
        Filter filter = new Filter("attachment.vpc-id").withValues(vpc.getVpcId());
    ...
    public List<RouteTable> getRouteTables(Vpc vpc) {
        Filter filter = new Filter("vpc-id").withValues(vpc.getVpcId());
